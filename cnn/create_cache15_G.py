...
def get_sample(vid, window_size = 20, out_size = 128, stride=4):
    ...
    if len(ws)>0:
        crop_size = int(4.5*max(np.mean(ws), np.mean(hs)))
    else:
        crop_size = out_size
    ...
    for i, ii in enumerate(window_size):
        if bboxes.sum() > 0:
            ...
            if path in image_dict:
                ...
                p_path = f'{vid}_{fr-2}'
                if p_path in image_dict:
                    p_img = image_dict[p_path]
                else:
                    p_img = image

                n_path = f'{vid}_{fr+2}'
                if n_path in image_dict:
                    n_img = image_dict[n_path]
                else:
                    n_img = image

                image = np.stack([p_img, image, n_img]).transpose(1,2,0)
...
os.makedirs('cache/cache15_G/', exist_ok=True)
results = []
vids = df.vid.unique()

window_size = [-72, -66, -60, -54, -48, -42, -36, -30, -24, -18, -13, -8, -4, -2, 0, 2, 4, 8, 13, 18, 24, 30, 36]
...
for vid in tqdm(vids):
    ...
    while 1:
        ...
        image_dict[kk] = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        frame_count +=1
    ...
    for i, row in tqdm(df1.iterrows()):
        ...
        fr_id = int(row['frame'])
        step = int(row['step'])
        e_images, e_masks1, e_empty_count = get_sample(e_vid, window_size = window_size, stride=4)
        s_images, s_masks1, s_empty_count = get_sample(s_vid, window_size = window_size, stride=4, out_size = 128)

        # # visualize
        # os.makedirs(f'draw/cache15_G/{idx1}_{idx2}_{fr_id}', exist_ok=True)
        ...
        #     cv2.imwrite(f'draw/cache15_G/{idx1}_{idx2}_{fr_id}/{ii}.jpg', e_img)



        path = f'cache/cache15_G/{vid}_{idx1}_{idx2}_{fr_id:04d}_{step}'
...
df.to_csv('train_cache15_G.csv', index=False)
